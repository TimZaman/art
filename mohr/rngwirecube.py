...
def draw_cube(ctx, x, size=64, offset=(0,0), p_do=0):
    ...
    draw_wire(ctx, x[0], x[1], p_do=p_do)
    draw_wire(ctx, x[1], x[2], p_do=p_do)
    draw_wire(ctx, x[2], x[3], p_do=p_do)
    draw_wire(ctx, x[3], x[0], p_do=p_do)
    draw_wire(ctx, x[4], x[5], p_do=p_do)
    draw_wire(ctx, x[5], x[6], p_do=p_do)
    draw_wire(ctx, x[6], x[7], p_do=p_do)
    draw_wire(ctx, x[7], x[4], p_do=p_do)
    draw_wire(ctx, x[0], x[4], p_do=p_do)
    draw_wire(ctx, x[1], x[5], p_do=p_do)
    draw_wire(ctx, x[2], x[6], p_do=p_do)
    draw_wire(ctx, x[3], x[7], p_do=p_do)
...
surface = cairo.PDFSurface ("out.pdf", canvas_size[0], canvas_size[1])
...
for cx in np.arange(-nx+1, nx):
    ...
    for cy in np.arange(-ny+1, ny):
        ...
        rm = rotation_matrix([0,1,1], cx * rot_scale)
        xr = np.matmul(xr, rm) 
